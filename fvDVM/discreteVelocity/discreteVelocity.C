...
void Foam::discreteVelocity::updateGHvol()
{
    gBarPvol_ = gVol_;
    hBarPvol_ = hVol_;
...
void Foam::discreteVelocity::updateGHInterpolation()
{
  ...
  // Pre-setup
    ...
    forAll(owner, facei)
    {
        ...
        if ((xii&Sf[facei]) >=  VSMALL) // comming from own
        {
            ...
            /// 0.5*dt ----->  dt
            // iGsurf[facei] = iGbarPvol[own] 
            //   + (iGbarPgrad[own]&(Cf[facei] - C[own] - xii*dt));

            // iHsurf[facei] = iHbarPvol[own]
            //   + (iHbarPgrad[own]&(Cf[facei] - C[own] - xii*dt));
...
void Foam::discreteVelocity::updateGHsurf()
{
    ...
    /// h = 0.5*time_.deltaT()   -------> h = time_.deltaT()
    //dimensionedScalar h = time_.deltaT();
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        else if (gSurfPatch.type() == "maxwellWall")
        {
            calculatedMaxwellFvPatchField<scalar>& rhoPatch = 
                refCast<calculatedMaxwellFvPatchField<scalar> >
                (dvm_.rhoVol().BOUNDARY_FIELD_REF[patchi]); //DEBUG
...
void Foam::discreteVelocity::updateFlux()   /// Update micro-flux
{
    ...
    gFlux_ = dimensionedScalar("0", gFlux_.dimensions(), 0);
    hFlux_ = dimensionedScalar("0", hFlux_.dimensions(), 0);
    // internal faces
    forAll(owner, facei)
    {
        const label own = owner[facei];
        const label nei = neighbour[facei];
        gFlux_[own] += ((xii&Sf[facei])*gSurf_[facei]); /// delete "*dt/V[own]"
        gFlux_[nei] -= ((xii&Sf[facei])*gSurf_[facei]); /// delete "*dt/V[own]"
        hFlux_[own] += ((xii&Sf[facei])*hSurf_[facei]); /// delete "*dt/V[own]"
        hFlux_[nei] -= ((xii&Sf[facei])*hSurf_[facei]); /// delete "*dt/V[own]"
    }
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        forAll(pOwner, pFacei)
        {
            ...
            gFlux_[own] += (xii&SfPatch[pFacei]) * gSurfPatch[pFacei]; /// delete "*dt/V[own]"
            hFlux_[own] += (xii&SfPatch[pFacei]) * hSurfPatch[pFacei]; /// delete "*dt/V[own]"
        }
    }
}

void Foam::discreteVelocity::updateequilibriumShakhov()
{
    equilibriumShakhov
    (
        gTildeVol_,
...
void Foam::discreteVelocity::forwardsweep()
{
    const labelUList& owner = mesh_.owner();
    const labelUList& neighbour = mesh_.neighbour();
    const vectorField& Sf = mesh_.Sf();
    //vectorField Sf = mesh_.Sf();
    const vectorField& C = mesh_.C();
    const scalarField& V = mesh_.V();
 
    dimensionedScalar Dt = dvm_.CFL() * time_.deltaT();
    volScalarField Di = 1./Dt + 1./dvm_.tauVol();
    label nei;
    vector xii = xi_.value();
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        if (type == "mixed" || type == "maxwellWall" || type == "cyclic" || type == "processor" || type == "DVMsymmetry")
        {
            //  check each boundary face in the patch
            forAll(gSurfPatch, facei)
            {
                // vector normal = SfPatch[facei]/mag(SfPatch[facei]);
                // if ((xii & normal) >  VSMALL )
                // {
                //     Di[faceCells[facei]] += ((xii & normal) * mag(SfPatch[facei]) / V[faceCells[facei]]);
                // }

                if ((xii & SfPatch[facei]) >  VSMALL )    /// V2
                {
                    Di[faceCells[facei]] += (xii & SfPatch[facei]) / V[faceCells[facei]];
                }
                // if(type == "cyclic" || type == "processor")
                // {
                //     //需要补充更新相邻格子内的信息
                //     // Dj_g += (xii & normal) * mag(Sf[faceIndex]) / V[cellI] * deltag_.BOUNDARY_FIELD_REF[patchi][facei];
                //     // Dj_h += (xii & normal) * mag(Sf[faceIndex]) / V[cellI] * deltah_.BOUNDARY_FIELD_REF[patchi][facei];
                // }
	        }
        }
	}
    ...
    for (label cellI = 0; cellI < mesh_.nCells(); ++cellI)
    {
        ...
        forAll(faces, facei)
        {
            ...
            if (faceIndex < mesh_.nInternalFaces())
            {
                ...
                if ((xii & normal) >  VSMALL )
                {
                    Di[cellI] += (((xii & normal) * mag(Sf[faceIndex]) / V[cellI]));
                }
                else
                {
                    Dj_g += (xii & normal) * mag(Sf[faceIndex]) / V[cellI] * deltag_[nei];
                    Dj_h += (xii & normal) * mag(Sf[faceIndex]) / V[cellI] * deltah_[nei];
                }
                
            }
        }
        deltag_[cellI] = ((gTildeVol_[cellI] - gVol_[cellI]) / dvm_.tauVol()[cellI] - gFlux_[cellI]/V[cellI] - Dj_g)/ Di[cellI];
        deltah_[cellI] = ((hTildeVol_[cellI] - hVol_[cellI]) / dvm_.tauVol()[cellI] - hFlux_[cellI]/V[cellI] - Dj_h)/ Di[cellI];
    }

    deltag_.correctBoundaryConditions();
    deltah_.correctBoundaryConditions();
}

void Foam::discreteVelocity::backwardsweep()
{
    const labelUList& owner = mesh_.owner();
    const labelUList& neighbour = mesh_.neighbour();
    const vectorField& Sf = mesh_.Sf();
...
void Foam::discreteVelocity::updateGHVol()
{
    ...
    deltag_ = dimensionedScalar("0", gVol_.dimensions(), 0.0);
    deltah_ = dimensionedScalar("0", hVol_.dimensions(), 0.0);
